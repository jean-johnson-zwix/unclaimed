...
function Header({ healthStatus, isBackendUp }: { healthStatus: HealthStatus; isBackendUp: boolean }) {
  return (
...
function Dashboard() {
  ...
  const visibleResults = useMemo(() => results.slice(0, MAX_RESULTS), [results]);

  return (
    <div className="flex min-h-[78vh] w-full flex-col gap-4">
      <Header healthStatus={healthStatus} isBackendUp={backendUp} />
        ...
        <div className="space-y-4 overflow-y-auto pr-1">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs text-slate-400">A single request is sent to the backend; repeated clicks are ignored while a request is in flight.</p>
            </div>
          </form>
          ...
          {summary ? (
            <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-4">
              ...
              <div className="mt-2 flex flex-wrap gap-2 text-sm text-slate-300">
                ...
                <span className="rounded-full bg-slate-800 px-2 py-1">Programs checked: {summary.programs_checked ?? 0}</span>
              </div>
            </div>
          ) : null}
